(function (factory) {
  ...
  $.extend($.summernote.plugins, {
    emoji: function (context) {
      ...
      var icons = {};
      var reverseIcons = {};
      var editorId = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
        /[xy]/g,
        function (c) {
      ...
      var addListener = function () {
        ...
        $body.on("mouseup", function (e) {
          ...
          if (
            $(target).hasClass("emoji-picker") ||
            $(target).hasClass("emoji-menu-tab")
          ) {
            return;
          }
          self.$panel.hide();
      ...
      var updateEmojisList = function (index) {
        ...
        if (index > 0) {
          $.each(self.icons, function (key, icon) {
            if (self.icons.hasOwnProperty(key) && icon[0] === index - 1) {
              $items.append(
      ...
      context.memo("button.emoji", function () {
        ...
        var button = ui.button({
          contents:
            '<i class="' +
            document.emojiButton +
            ' emoji-picker-container emoji-picker"></i>',
          click: function () {
            if (document.emojiSource === undefined) document.emojiSource = "";
            if (document.emojiType === undefined) document.emojiType = "";
